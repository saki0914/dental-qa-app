...
test("@authenticated Emulator上でログイン後の問題CRUDと画像暗記を永続化する", async ({ page }) => {
  ...
  await page.locator("#subjectFilterSummary").click();
  await page.locator('#subjectFilterChecklist input[value="E2E歯科学"]').check();
  await page.locator("#subjectFilterSummary").click();
  await page.locator("#primarySubcategorySummary").click();
  await page.locator('#primarySubcategoryChecklist input[value="E2E"]').check();
  await page.locator("#primarySubcategorySummary").click();
  ...
  await page.locator("#subjectFilterSummary").click();
  await page.locator('#subjectFilterChecklist input[value="E2E補綴学"]').check();
  await page.locator("#subjectFilterSummary").click();
  await page.locator("#primarySubcategorySummary").click();
  await page.locator('#primarySubcategoryChecklist input[value="別章"]').check();
  await page.locator("#primarySubcategorySummary").click();
  await expect(page.locator("#subjectFilterSummary")).toContainText("2件選択");
  await expect(page.locator("#primarySubcategorySummary")).toContainText("2件選択");
  await expect(page.locator("#totalCount")).toHaveText("4");
  if (process.env.VISUAL_QA) await page.locator("#subjectFilterSummary").click();
  ...
  await page.locator("#pdfTagInput").fill("E2E,画像暗記");
  ...
  await expect(page.locator("#pdfTagFilterSelect")).toContainText("E2E");
  await expect(page.locator("#tab-pdf .subcat-chip")).toHaveCount(0);
  await page.locator("#pdfTagFilterSelect").selectOption("E2E");
  ...
  await captureVisualPair(page, "image-memory");
