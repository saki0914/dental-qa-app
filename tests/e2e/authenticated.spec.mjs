...
test("@authenticated Emulator上でログイン後の問題CRUDと画像暗記を永続化する", async ({ page }) => {
  ...
  await page.locator('#pdfEditTableBody tr:has-text("E2E画像教材")').click();
  ...
  await page.locator("#pdfEditModeBtn").click();
  await page.locator('#pdfEditTableBody tr:has-text("E2E画像教材更新")').click();
  page.once("dialog", dialog => dialog.accept());
  await page.locator("#deletePdfBtn").click();
  await expect(page.locator("#pdfEditTableBody")).not.toContainText("E2E画像教材更新");
  await expect(page.locator("#pdfEditStatus")).toContainText("画像教材を削除しました", { timeout: 20_000 });

  await page.locator('#pdfEditTableBody tr:has-text("E2E別教科教材")').click();
  page.once("dialog", dialog => dialog.accept());
  await page.locator("#deletePdfBtn").click();
