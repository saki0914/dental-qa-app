...
test("@authenticated Emulator上でログイン後の問題CRUDと画像暗記を永続化する", async ({ page }) => {
  ...
  const ownerIdToken = await createEmulatorUser(email, password);
  ...
  await expect(page.locator("#saveCloudBtn, #loadCloudBtn, #saveCloudBtn2")).toHaveCount(0);
  ...
  await page.locator("#bulkImportFile").setInputFiles({
    ...
    buffer: Buffer.from(JSON.stringify([
      ...
      {
        subject: "E2E歯科学",
        subcategories: ["E2E", "Emulator", "不正パス"],
        question: "不正な画像パスを持つ問題",
        answers: ["削除をスキップ"],
        explanation: "所有者の異なるStorageパスは削除しない",
        imagePath: "users/another-user/questions/foreign/unsafe.png",
  ...
  await expect(page.locator("#manageFullscreenBtn")).toHaveCount(0);
  ...
  await expect(page.locator("#questionTableBody")).toContainText("不正な画像パスを持つ問題");
  ...
  const bulkQuestionImageUrl = await page.locator("#imagePreview").getAttribute("src");
  expect(bulkQuestionImageUrl).toBeTruthy();
  const bulkQuestionImageLocation = new URL(bulkQuestionImageUrl);
  expect(bulkQuestionImageLocation.origin).toBe("http://127.0.0.1:9199");
  expect(bulkQuestionImageLocation.pathname).toMatch(/^\/v0\/b\/demo-dental-qa\.firebasestorage\.app\/o\//);
  const storageRequestOptions = {
    headers: { Authorization: `Bearer ${ownerIdToken}` }
  };
  const existingImageResponse = await page.request.get(bulkQuestionImageUrl, storageRequestOptions);
  expect(existingImageResponse.status()).toBe(200);
  ...
  await expect(page.locator("#totalCount")).toHaveText("5");
  ...
  await page.locator("#tabBtnManage").click();
  await page.locator("#clearFormBtn").click();
  const forcedFailureMessages = [];
  let storageDeleteRequests = 0;
  await page.route("**/v0/b/**/o/**", async route => {
    if (route.request().method() === "DELETE") {
      storageDeleteRequests += 1;
      await route.fulfill({
        status: 403,
        contentType: "application/json",
        body: JSON.stringify({ error: { code: 403, message: "forced Storage delete failure" } })
      });
      return;
    }
    await route.continue();
  });
  const forcedFailureDialogHandler = async dialog => {
    forcedFailureMessages.push(dialog.message());
    await dialog.accept();
  };
  page.on("dialog", forcedFailureDialogHandler);

  await page.locator('[data-manage-row]:has-text("削除確認用の問題") [data-delete-question]').check();
  await page.locator('[data-manage-row]:has-text("更新後の画像付き問題") [data-delete-question]').check();
  await page.locator('[data-manage-row]:has-text("不正な画像パスを持つ問題") [data-delete-question]').check();
  await page.locator("#manageDeleteCheckedBtn").click();
  await expect.poll(() => forcedFailureMessages.length, { timeout: 20_000 }).toBe(2);
  page.off("dialog", forcedFailureDialogHandler);
  await page.unroute("**/v0/b/**/o/**");

  expect(forcedFailureMessages[1]).toContain("1件の問題を削除しました");
  expect(forcedFailureMessages[1]).toContain("2件は画像を削除できなかったため、問題データを復元しました");
  expect(forcedFailureMessages[1]).toContain("更新後の画像付き問題");
  expect(forcedFailureMessages[1]).toContain("不正な画像パスを持つ問題");
  expect(storageDeleteRequests).toBe(1);
  await expect(page.locator("#questionTableBody")).not.toContainText("削除確認用の問題");
  await expect(page.locator("#questionTableBody")).toContainText("更新後の画像付き問題");
  await expect(page.locator("#questionTableBody")).toContainText("不正な画像パスを持つ問題");
  await waitForCloudSave(page);

  await page.locator("#manageClearDeleteSelectionBtn").click();
  const normalImageDeleteTarget = page.locator('[data-manage-row]:has-text("一括登録した画像付き問題")');
  await normalImageDeleteTarget.locator("[data-delete-question]").check();
  page.once("dialog", dialog => dialog.accept());
  await page.locator("#manageDeleteCheckedBtn").click();
  await expect(page.locator("#questionTableBody")).not.toContainText("一括登録した画像付き問題");
  await waitForCloudSave(page);
  const deletedImageResponse = await page.request.get(bulkQuestionImageUrl, storageRequestOptions);
